...
def main(config: dict):
    ...
    data_transform = transforms.Compose([
        transforms.Resize(224), # replaced Padding with Resize logic 
        transforms.ToTensor(),
        transforms.Normalize(mean=mean, std=std)
    ])
    # no data augmentation or other logic implemented

    # Create the datasets
    train_dataset = DataClass(split='train', transform=data_transform, download=False, as_rgb=True, root=config['data_path'])
    val_dataset = DataClass(split='val', transform=data_transform, download=False, as_rgb=True, root=config['data_path'])
    test_dataset = DataClass(split='test', transform=data_transform, download=False, as_rgb=True, root=config['data_path'])
...
if __name__ == '__main__':
    ...
    # Create from the config file with list elements a config file with unique elements to run the normal train logic and log the training output to an file and to the console
    # Save the original stdout so we can restore it later
    original_stdout = sys.stdout
    #Assign the file path for the final log output file
            ...
            for training_procedure in main_config['training_procedures']:
                for architecture in main_config['architectures']:
                    for seed in main_config['seeds']:
                        # Select encoder LR based on architecture type
                        config = {
                            "data_path": main_config['data_path'],
                            "output_path": main_config['output_path'],
                            "log_base_output_path": main_config['log_base_output_path'],
                            "iterations": main_config['iterations'],
                            # Store the selected LR
                            "learning_rate_encoder": main_config['learning_rate_encoder'],
                            "learning_rate_decoder": main_config['learning_rate_decoder'],
                        ...
                        # Redirect stdout to DualOutput (console and log file)
